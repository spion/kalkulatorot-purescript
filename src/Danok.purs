...
presmetajDanoci osnova d = { dld10: od d.dld10 osnova }
...
bruto2neto bruto =
  let
    ...
    pridonesi = presmetajPridonesi osnovica procentiPridonesi

    vkupnoPridonesi = sumaPridonesi pridonesi

    dldOsnova = bruto - vkupnoPridonesi - licnoOsloboduvanje

    dldOsnova10 = dldOsnova

    danoci = presmetajDanoci dldOsnova procentiDanoci
...
findBruto netoVal =
  let
    val = clamp minNeto maxSafeInt netoVal
  in
...
binSearch searchValue lo hi =
  let
    mid = lo + floor (toNumber (hi - lo) / 2.0)

    value = (bruto2neto mid).neto
  in
...
neto2bruto val = bruto2neto (findBruto val)
